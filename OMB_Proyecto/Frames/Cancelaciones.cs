...
namespace OMB_Proyecto.Frames
{
    public partial class Cancelaciones : Form
    {
        ...
        {
            InitializeComponent();
            TablaCancelaciones.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;
        }
        ...
        private void Search_Click(object sender, EventArgs e)
        {
            if (Seleccion.SelectedIndex >= 0)
            {
                ...
                db.LlenarTabla_PolCancel(TablaCancelaciones, selec);
                selec = 0;
            }
        }
        ...
        private void TablaCancelaciones_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            if (this.TablaCancelaciones.Columns[e.ColumnIndex].Name == "Estado")
            {
                try
                {
        ...
        private void Descargar_Click(object sender, EventArgs e)
        {
            ...
            PdfPTable pdfTable = new PdfPTable(TablaCancelaciones.ColumnCount);
            ...
            //Adding Header row
            foreach (DataGridViewColumn column in TablaCancelaciones.Columns)
            {
            ...
            //Adding DataRow
            foreach (DataGridViewRow row in TablaCancelaciones.Rows)
            {
                try
                {
            ...
            save.FileName = "Reporte polizas CANCELADAS";
            ...
            if (save.ShowDialog() == DialogResult.OK)
            {
                ...
                pdfDoc.Open();
                pdfDoc.AddTitle("REPORTE POLIZAS CANCELADAS");
                pdfDoc.Add(new Paragraph("REPORTE POLIZAS CANCELADAS POR MES", FontFactory.GetFont("ARIAL", 30, iTextSharp.text.Font.BOLD)));
